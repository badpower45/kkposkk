...
async function checkConnection(): Promise<boolean> {
  ...
  try {
    ...
    if (useLocalFallback) {
      console.log('ℹ️ نظام Supabase غير متاح - سيتم استخدام التخزين المحلي (الوضع غير المتصل)');
    }
    
    return !useLocalFallback;
  } catch (error) {
    console.log('ℹ️ تعذر الاتصال بـ Supabase - تفعيل وضع التخزين المحلي تلقائياً');
    connectionChecked = true;
...
async function apiCall<T>(endpoint: string, options?: RequestInit): Promise<T> {
  ...
  try {
    ...
    console.log('ℹ️ التبديل إلى التخزين المحلي بسبب خطأ في الاتصال');
    useLocalFallback = true;
    return handleLocalFallback<T>(endpoint, options);
  }
}
